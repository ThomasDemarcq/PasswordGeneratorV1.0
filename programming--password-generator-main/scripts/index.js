...
function generatePassword(length) {
  /*
    ...
    for(let i = 0;i < length; i++){
      ...
      avMaj = Math.round(Math.random())
      if(avMaj == 1){
        A = randomUpperCase()
        pass[i] = A
      }
      avNb = Math.round(Math.random())
      if(avNb  == 1){
        NB = randomNumber()
        pass[i] = NB
      }
      avSpe = Math.round(Math.random())
      if(avSpe  == 1){
        Spe = randomSymbol()
        pass[i] = Spe
      }
